...
class dxl_controller:
    def __init__(self, id):
        ...
        if self.portHandler.openPort():
            print('{0}:Succeeded to open the port'.format(self.DXL_ID))
        else:
        ...
        if self.portHandler.setBaudRate(self.BAUDRATE):
            print('{0}:Succeeded to change the baudrate'.format(self.DXL_ID))
        else:
        ...
        self.dxl_comm_result, self.dxl_error = self.packetHandler.write1ByteTxRx(self.portHandler, self.DXL_ID, self.ADDR_OPERATING_MODE, self.OperatingMode)
        ...
        if self.dxl_comm_result != COMM_SUCCESS:
            ...
            print('{0}:Dynamixel has been successfully connected'.format(self.DXL_ID))

    def controllVelocity(self, vel):
        self.dxl_comm_result, self.dxl_error = self.packetHandler.write4ByteTxRx(self.portHandler, self.DXL_ID, self.ADDR_GOAL_VELOCITY, vel)
    ...
    def controllPosition(self):
        #print("Press any key to continue! (or press ESC to quit!)")
        #if getch() == chr(0x1b):

        self.dxl_comm_result, self.dxl_error = self.packetHandler.write4ByteTxRx(self.portHandler, self.DXL_ID, self.ADDR_GOAL_POSITION, self.dxl_goal_position[self.index])
        ...
        print("[ID:%03d] GoalPos:%03d  PresPos:%03d" % (self.DXL_ID, 0, self.dxl_present_position))
    ...
    def release(self):
        ...
        self.dxl_comm_result, self.dxl_error = self.packetHandler.write1ByteTxRx(self.portHandler, self.DXL_ID, self.ADDR_TORQUE_ENABLE, 0)
        # Close port
        print('{0}:close port'.format(self.DXL_ID))
